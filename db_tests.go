...
func withDB(t TestingT, mdb minisql.DB, fn func(db DB)) {
...
func get[T any](t TestingT, q Queryable, path string) T {
	result, err := Get[T](q, path)
	require.NoError(t, err)
	return result
}

func rget[T any](t TestingT, q Queryable, path string) *Raw[T] {
	result, err := RGet[T](q, path)
	require.NoError(t, err)
	return result
}

func list[T any](t TestingT, q Queryable, query *QueryParams) []*Item[T] {
	result, err := List[T](q, query)
	require.NoError(t, err)
	return result
}

func listPaths(t TestingT, q Queryable, query *QueryParams) []string {
	result, err := ListPaths(q, query)
	require.NoError(t, err)
	return result
}

func rlist[T any](t TestingT, q Queryable, query *QueryParams) []*Item[*Raw[T]] {
	result, err := RList[T](q, query)
	require.NoError(t, err)
	return result
}

func search[T any](t TestingT, q Queryable, query *QueryParams, search *SearchParams) []*SearchResult[T] {
	result, err := Search[T](q, query, search)
	require.NoError(t, err)
	return result
}

func rsearch[T any](t TestingT, q Queryable, query *QueryParams, search *SearchParams) []*SearchResult[*Raw[T]] {
